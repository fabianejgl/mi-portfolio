...
class TestPropios : public testing::Test {
protected:
    Casilla pos0, pos1, pos2, pos3, pos4, pos5, pos6, pos7, pos8, pos9;
    set<Casilla> posicion1, posicion2, posiciones234, posiciones01234, posiciones56789;
    set<int> h1, v1, h2, v2, h3, v3, h4, v4, h12, v12, h1234, v1234;
...
TEST_F(TestPropios, casa_mata_comercio_de_mismo_nivel) {

    /*
    los siguientes tests verifican que en todos los casos posibles de union de simcitys,
    si se superponen una casa y un comercio, en el simcity resultado queda el comercio.
    esto se ve en la linea 26 del archivo Constr.cpp, en la que se recorren las construcciones de los simcitys unidos,
    y se agregan si no habia nada en esa ubicacion del simcity raiz, o si en el simcity raiz esa ubicacion tenia un comercio
    */

    aed2_Servidor s;
    s.nuevaPartida(jugador1, {}, {});
    s.nuevaPartida(jugador2, {}, {});

    /// casa superpuesta con comercio, la casa en el primer simcity de la union
    s.agregarCasa(jugador1, {1,1});
    s.agregarComercio(jugador2, {1,1});

    /// casa superpuesta con comercio, la casa en el segundo simcity de la union
    s.agregarComercio(jugador1, {2,2});
    s.agregarCasa(jugador2, {2,2});

    s.avanzarTurno(jugador1);
    s.avanzarTurno(jugador2);
    s.unir(jugador1, jugador2);

    set<Casilla> ambas = {{1,1},{2,2}};
    set<Casilla> vacio = {};
    EXPECT_EQ(s.casas(jugador1), ambas);
    EXPECT_EQ(s.comercios(jugador1), vacio);
}

TEST_F(TestPropios, casa_mata_comercio_de_mayor_nivel) {
...
TEST_F(TestPropios, casa_mata_comercio_de_menor_nivel) {

    aed2_Servidor s;
    s.nuevaPartida(jugador1, {}, {});
    s.nuevaPartida(jugador2, {}, {});

    s.agregarCasa(jugador1, {11,11});
    s.agregarCasa(jugador2, {22,22});
    ...
    s.agregarComercio(jugador2, {11,11});
    s.agregarComercio(jugador1, {22,22});

    s.avanzarTurno(jugador1);
    s.avanzarTurno(jugador2);
    s.unir(jugador1, jugador2);

    set<Casilla> ambas = {{11,11},{22,22}};
    set<Casilla> vacio = {};
    EXPECT_EQ(s.casas(jugador1), ambas);
    EXPECT_EQ(s.comercios(jugador1), vacio);
}

TEST_F(TestPropios, stress_test_construcciones) {
    aed2_Servidor s;
    s.nuevaPartida(jugador1, {}, {});
    s.nuevaPartida(jugador2, {}, {});

    for(int i = 0; i < 10000; i = i+2){
        s.agregarCasa(jugador1, {i,i});
        s.agregarComercio(jugador1, {i+1,i+1});
    }
    // agrego casas en las posiciones (j,j), donde j es un numero par,
    // y comercios en las posiciones (k,k), donde k es un numero impar,
    // con 0 <= k,j < 10000;

    for(int i = 10000; i < 20000; i = i+2){
        s.agregarCasa(jugador2, {i,i});
        s.agregarComercio(jugador2, {i+1,i+1});
    }

    // hago lo mismo en el simcity 2, pero entre el 10000 y el 20000
    ...
    for(int i = 0; i < 20000; i = i+2){
        casas.insert({i,i});
        comercios.insert({i+1,i+1});
    }

    EXPECT_EQ(s.casas(jugador1), casas);
    EXPECT_EQ(s.comercios(jugador1), comercios);
}

TEST_F(TestPropios, stress_test_uniones) {
    aed2_Servidor s;

    for(int i = 0 ; i < 1000 ; i++){
        string nombre = to_string(i);
        s.nuevaPartida(nombre, {}, {});
        s.agregarCasa(nombre, {i,i});
        s.avanzarTurno(nombre);
    }    // creo 1000 simcitys con nombres del 0 al 999
         // el simcity i va a tener una casa en la posicion (i,i)

    for(int i = 0; i < 999; i++){
        string primero = to_string(i);
        string segundo = to_string(i+1);
        s.unir(segundo,primero);
    }     // hago unir(1,0), despues unir(2,1), hasta unir(999,9998)
    ...
    EXPECT_EQ(s.casas(to_string(999)), casas);
}
